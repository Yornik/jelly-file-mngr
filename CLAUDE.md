...
2. `aside.classify_aside(file)` — checked before anything else; aside files are smart-routed to `dest/.aside/<relative-path>` and never enter the TMDB flow
...
- Aside quarantine: `dest/.aside/<relative-from-source>/filename` (dot-prefix so Jellyfin ignores it)
